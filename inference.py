...
class Inference:
    ...
    def start_inference(self):
        ...
        os.makedirs(os.path.join(self.name, 'dec_img_B'), exist_ok=True) 
        os.makedirs(os.path.join(self.name, 'img_A'), exist_ok=True)
        os.makedirs(os.path.join(self.name, 'img_B'), exist_ok=True)
        os.makedirs(os.path.join(self.name, 'cd'), exist_ok=True)
        ...
        for batch in tqdm(self.loader_val): # note: iter_idx is not from 0 if resume training
            ...
            item = batch['item'][0]
            torchvision.utils.save_image( dec, os.path.join(self.name, 'dec_img_B', f'{item}'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
            torchvision.utils.save_image( batch["img_A"], os.path.join(self.name, 'img_A', f'{item}'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
            torchvision.utils.save_image( batch["img_B"], os.path.join(self.name, 'img_B', f'{item}'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--yaml_file", type=str,  default="configs/levir_cd.yaml", help="paths to base configs.")
    ...
    config.train_dataset_names.LEVIR.random_crop = False
    config.train_dataset_names.LEVIR.random_flip = False
