...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, sem_feature):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, objs, sem_feature)
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    def __init__(
        ...
        # self.context_compressor = nn.Conv1d(in_channels=248, out_channels=77, kernel_size=1, stride=1, padding=0)  # wzy

        # self.cross_objs_context = CrossAttention(query_dim=context_dim, key_dim=context_dim, value_dim=context_dim, heads=num_heads, dim_head=context_dim // num_heads)  # wzy, _flir_longclip_cross_dual2_lpips
        # self.cross_objs_context = GatedCrossAttentionDense(query_dim=context_dim, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads, d_head=context_dim//num_heads)  # wzy, _flir_longclip_cross_dual2_lpips
    ...
    def forward(self, input):
        ...
        if ("grounding_input" in input):
            grounding_input = input["grounding_input"]  # {"boxes":boxes, "masks":masks, "positive_embeddings":positive_embeddings} / {"sem":sem, "mask":mask}
        else: 
            # Guidance null case
        ...
        objs, sem_feature = self.position_net( **grounding_input )  # **grounding_input：img_A, label, mask
        ...
        # Text input 
        context = input["context"].float()  # wzy +.float(),  (batch_size, 248, 768)
        # print("context___:", context.dtype, context.shape)

        # Start forwarding 
        hs = []
        for module in self.input_blocks:
            h = module(h, emb, context, objs, sem_feature)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, sem_feature)
