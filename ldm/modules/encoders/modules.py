...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cuda", max_length=77):
        super().__init__()
        self.tokenizer = CLIPTokenizer.from_pretrained(version)   # If you do not have the access to huggingface, you can replace "version" with the path of the downloaded clip-vit-large-patch14
    ...
    def forward(self, text, return_pooler_output=False):
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        tokens = batch_encoding["input_ids"].to(self.device)  # torch.Size([batch_size, 77])
        outputs = self.transformer(input_ids=tokens)

        z = outputs.last_hidden_state  # torch.Size([batch_size, 77, 768])
