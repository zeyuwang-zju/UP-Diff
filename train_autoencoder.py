...
class Trainer_Autoencoder:
    ...
    def run_one_step(self, batch):
        ...
        loss_mse = torch.nn.functional.mse_loss(dec, batch["img_B"])
        loss_lpips = self.lpips_loss(dec, batch["img_B"]).mean()
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            torchvision.utils.save_image( torch.cat([batch["img_B"], dec], dim=0), save_path, nrow=self.config.batch_size, normalize=True, scale_each=True, range=(-1,1) )
...
if __name__ == "__main__":
    ...
    # CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train_autoencoder.py  --yaml_file configs/levir_cd.yaml  --DATA_ROOT /disk3/zeyu/LEVIR-CD/   --batch_size 2   --save_every_iters 1000   --name levir
    # CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train_autoencoder.py  --yaml_file configs/sysu_cd.yaml  --DATA_ROOT /disk3/zeyu/SYSU-CD/   --batch_size 2   --save_every_iters 1000   --name sysu