...
class Trainer:
    def __init__(self, config):
        ...
        for name, p in self.model.named_parameters():
            ...
            if ("transformer_blocks" in name) and ("fuser" in name):
                ...
                params.append(p) 
                trainable_names.append(name)
            elif "proj_sem" in name:
                params.append(p)
                trainable_names.append(name)
            elif "gated_conv" in name:
                params.append(p)
                trainable_names.append(name)
            else:
    ...
    def get_input(self, batch):
        ...
        z = self.autoencoder.encode( batch["img_B"] )
        context = self.text_encoder.encode( batch["caption"]  )  # torch.Size([batch_size, 248, 768])
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            batch_here = batch["img_A"].shape[0]
            ...
            dec = autoencoder_wo_wrapper.decode(samples)
            dec = torch.clamp(dec, min=-1, max=1).cpu()

            if "boxes" in batch:
                real_images_with_box_drawing = [] # we save this durining trianing for better visualization
                for i in range(batch_here):
                    temp_data = {"image": batch["img_A"][i], "boxes":batch["boxes"][i]}
                ...
                real_images_with_box_drawing = [] # we save this durining trianing for better visualization
                for i in range(batch_here):
                    real_images_with_box_drawing.append((batch["img_A"][i]*0.5 + 0.5).cpu())
                    real_images_with_box_drawing.append((torch.cat((batch["label"][i], batch["label"][i], batch["label"][i]), dim=0)).cpu())
                    real_images_with_box_drawing.append((batch["img_B"][i]*0.5 + 0.5).cpu())
                    real_images_with_box_drawing.append((dec[i]*0.5 + 0.5).cpu())

                real_images_with_box_drawing = torch.stack(real_images_with_box_drawing)
            
            save_path = os.path.join(self.name, str(iter_name).zfill(8)+'.png')
            torchvision.utils.save_image( real_images_with_box_drawing, save_path, nrow=4 )

 

        ckpt = dict(model = model_wo_wrapper.state_dict(),
                    # text_encoder = self.text_encoder.state_dict(), # long-clip权重会自动加载，不需要保存
...
if __name__ == "__main__":
    ...
    parser.add_argument("--yaml_file", type=str,  default="configs/levir_cd.yaml", help="paths to base configs.")  # 框生图用flickr_text.yaml
    ...
    # CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train_diffusion.py  --yaml_file=configs/levir_cd.yaml  --DATA_ROOT /disk3/zeyu/LEVIR-CD/   --batch_size=4   --save_every_iters 1000   --name levir
    # CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 train_diffusion.py  --yaml_file=configs/sysu_cd.yaml  --DATA_ROOT /disk3/zeyu/SYSU-CD/   --batch_size=4   --save_every_iters 1000   --name sysu
